---
title: "Study Fleet and Observer Data Pull for Black Sea Bass"
author: "Andy Jones"
date: "10/13/2021"
...
# Description
This is an example of a github document knitted from the Rmd file. It takes one extra file to make a database connection, but otherwise is self contained. It goes to the observer database as well as the Study Fleet database to find information that is available. Here all gears are brought in. The focus is on pulling information that could be useful for a CPUE estimate for black seabass.
...
# CRB data pull
Pulling the CRB data as well as a couple of other tables. This includes the CRB trip, effort, and catch data. Conversion factors from CFDBS and VTR gear codes. Also a VERS trip list for the CRB data. At the bottom it sticks all of these things together into a data frame to work from.
...
Converting the decimal degree minutes format in the CRB data to decimal degrees.

```{r Making functions to convert the CRB lat/lon data}
...
Pulling the OB and ASM data tables for trip, haul, and catch data. This is just for trawl gears here.

```{r Pulling the observer data}
...
Only bringing in specific columns and records from OBHAU. Bringing the ASM and OB tables together.

```{r Subsetting the Observer data, message=FALSE, warning=FALSE, include=FALSE}
#Bringing together the observer and ASM trip data
#Joining the haul data from ASM and OBS data sets
OBHAU_SUB <- OBHAU %>% dplyr::select(TRIPID,LINK1,LINK3,HAULNUM,NEGEAR,
                              DATEHBEG,TIMEHBEG,DATEHEND,TIMEHEND,
                              GIS_LATHBEG,GIS_LATHEND,GIS_LONHBEG,GIS_LONHEND,
                              DATESBEG,TIMESBEG,DATESEND,TIMESEND,
                              GIS_LATSBEG,GIS_LATSEND,GIS_LONSBEG,GIS_LONSEND,
                              SOAKDUR,HAULDUR,AREA,DEPTH,TARGSPEC1)

ASMHAU_SUB <- ASMHAU %>% dplyr::select(TRIPID,LINK1,LINK3,HAULNUM,NEGEAR,
                              DATEHBEG,TIMEHBEG,DATEHEND,TIMEHEND,
                              GIS_LATHBEG,GIS_LATHEND,GIS_LONHBEG,GIS_LONHEND,
                              DATESBEG,TIMESBEG,DATESEND,TIMESEND,
                              GIS_LATSBEG,GIS_LATSEND,GIS_LONSBEG,GIS_LONSEND,
...
#Binding the ASM and OB haul data 
ASMOBHAU_SUB <- rbind(OBHAU_SUB,ASMHAU_SUB)

#Subsetting the trip data
OBTRP_SUB <- OBTRP %>% dplyr::select(TRIPID,HULLNUM1,LINK1,PERMIT1,DATESAIL,TRIPEXT)
ASMTRP_SUB <- ASMTRP %>% dplyr::select(TRIPID,HULLNUM1,LINK1,PERMIT1,DATESAIL,TRIPEXT)

#Joining some haul and trip data
OBHAU_SUB_TRIP <- left_join(OBHAU_SUB,OBTRP_SUB)
ASMHAU_SUB_TRIP <- left_join(ASMHAU_SUB,ASMTRP_SUB)

#Joining the two
ASMOBHAU_SUB_TRIP <- rbind(ASMHAU_SUB_TRIP,OBHAU_SUB_TRIP)

#Filtering out trip extensions for trips we don't need (X is generally complete, C is complete gillnet trips)
ASMOBHAU_SUB_TRIP <- ASMOBHAU_SUB_TRIP %>% filter(TRIPEXT=='X' | TRIPEXT=='C')

#Making a single catch dataframe
ASM_OB_CATCH_SUB <- rbind(OBSPP,ASMSPP %>% dplyr::select(-ASMTEST_STATUS)) %>% left_join(.,OBSPEC %>% dplyr::select(NESPP4,SPECIES_ITIS,COMNAME) %>% distinct(),by='NESPP4') %>% filter(LINK1 %in% ASMOBHAU_SUB_TRIP$LINK1) %>% left_join(ASMOBHAU_SUB_TRIP)

```

# Polishing the Obs data
Converting landed weight to live weight and summarizing catch for the species of interest (discards, kept, and total). 

```{r Polishing the Obs data}
...
Changing the depth variable to be in fathoms, converting landed weight to live weight, summarizing catch for the species of interest (discards, kept, and total). 

```{r Polishing the CRB data}
...
Making the columns the same so I can bring things together with just a simple row bind.

```{r Combining the two data set}
...
Matching observer and cooperative research trips by permit and date, then comparing the total catch of BSB on a given trip between the two data sources. Need to make sure the observer trips without a permit number get accounted for too. Needs improvement.
...
This come from GARFO and is messy. Each vessel has an entry per year in the VPS vessels table. Need to run past Ben and co.
...
# Making more plots of the data we've brought together
Second looking at the month when black sea bass were caught.

```{r}
#plotting things out just to see where the records are from
a <- ggplot() + 
  geom_bar(data=bind_rows(plot_data_comb_ves_1,plot_data_comb_ves_2) %>% filter(SUM_BSB_CATCH>0) %>% 
             group_by(MONTH,source,YEAR) %>% tally(),
                  aes(x=YEAR,y=n,fill=as.factor(MONTH)),
                  stat='identity') +
  facet_wrap(~source) +
  labs(title='BSB catch in SF/OB by fishing MONTH',subtitle = 'Number of records',x='Year',y='Count') +
  theme(legend.position = "none")

b <- ggplot() + 
  geom_bar(data=bind_rows(plot_data_comb_ves_1,plot_data_comb_ves_2) %>% filter(SUM_BSB_CATCH>0),
                  aes(x=YEAR,fill=as.factor(MONTH)),
                  position = "fill") +
  facet_wrap(~source) + scale_y_continuous(labels = scales::percent) +
  labs(title='BSB catch in SF/OB by fishing MONTH',subtitle = 'Number of records as percent',x='Year',y='Percent') +
  theme(legend.position = "none")

c <- ggplot() + 
  geom_bar(data=bind_rows(plot_data_comb_ves_1,plot_data_comb_ves_2) %>% filter(SUM_BSB_CATCH>0) %>% 
             group_by(MONTH,source,YEAR) %>% summarise(BSB_sum=sum(SUM_BSB_CATCH,na.rm=TRUE)) %>%
             ungroup(),
                  aes(x=YEAR,y=BSB_sum,fill=as.factor(MONTH)),
                  stat='identity') +
  facet_wrap(~source) +
  labs(title='BSB catch in SF/OB by fishing MONTH',subtitle = 'Weight summary (lbs)',x='Year',y='Percent',fill='Fishing MONTH')
