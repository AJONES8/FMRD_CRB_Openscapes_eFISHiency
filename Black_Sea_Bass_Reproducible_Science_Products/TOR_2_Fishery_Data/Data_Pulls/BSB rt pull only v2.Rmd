...
```{r Setting up DB connection include=FALSE}
...
#OBHAU <- dbGetQuery(con.nova, "select * from OBDBS.OBHAU where YEAR > 2018 and YEAR < 2020")
#ASMHAU <- dbGetQuery(con.nova, "select * from OBDBS.ASMHAU where YEAR > 2018 and YEAR < 2020")
...
ASM_OB_CATCH_SUB_edit_sum <- ASM_OB_CATCH_SUB_edit_cf %>%
  mutate(BSB_KEPTWT = case_when(SPECIES_ITIS==	'172877' & CATDISP == '1' ~ HAIL_AMOUNT_LB,TRUE ~ 0),
         BSB_DISCARDTWT = case_when(SPECIES_ITIS==	'172877' & CATDISP == '0' ~ HAIL_AMOUNT_LB,TRUE ~ 0),
...
pulled_data_cf_sum <-pulled_data_edit_cf %>%
  mutate(BSB_KEPTWT = case_when(SPECIES_ITIS==	'172877' & DISPOSITION_CODE == '011' ~ HAIL_AMOUNT_LB,TRUE ~ 0),
         BSB_DISCARDTWT = case_when(SPECIES_ITIS==	'172877' & DISPOSITION_CODE == '100' ~ HAIL_AMOUNT_LB,TRUE ~ 0),
         SUM_BSB_CATCH = BSB_KEPTWT + BSB_DISCARDTWT)
...
Making the columns the same so I can bring things together.
...
Matching observer and cooperative research trips by permit and date, then comparing the total catch of BSB on a given trip between the two data sources.
...
##Adding the wide catch elements
...
#bind_rows(plot_data_comb_ves_1,plot_data_comb_ves_2) %>% skim()
#bind_rows(plot_data_comb_ves_1,plot_data_comb_ves_2) %>% filter(is.na(mesh_size)) %>% group_by(source,gear_code_vtr,mesh_size) %>% count()

#Actually writing a data file
#write_rds(bind_rows(plot_data_comb_ves_1,plot_data_comb_ves_2),'BSB_sf_ob_v3.rds')
...
ggplot() + 
  stat_summary_2d(data=bind_rows(plot_data_comb_ves_1,plot_data_comb_ves_2) %>% filter(SUM_BSB_CATCH>0),
                  aes(y=end_lat,x=end_lon,z=log(SUM_BSB_CATCH+1)),
                  fun="mean",na.rm = FALSE,binwidth = c(0.08333,0.08333),alpha=0.75) +
  facet_wrap(~source)
