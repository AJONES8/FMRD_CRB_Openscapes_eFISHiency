...
# Looking at what we've brought together

```{r}
library(skimr)
plot_data_comb %>% skim()

plot_data_comb %>% .$SUM_BSB_CATCH %>% skim()

plot_data_comb %>% filter(SUM_BSB_CATCH>0) %>% skim()
...
bind_rows(plot_data_comb_ves_1,plot_data_comb_ves_2) %>% .$SUM_BSB_CATCH %>% qplot()

bind_rows(plot_data_comb_ves_1,plot_data_comb_ves_2) %>% .$SUM_BSB_CATCH %>% 

plot_data_comb %>% dplyr::select(trip_id,SUM_BSB_CATCH) %>% inner_join(.,bind_rows(plot_data_comb_ves_1,plot_data_comb_ves_2) %>% dplyr::select(trip_id,SUM_BSB_CATCH_v2=SUM_BSB_CATCH),by='trip_id') %>% sample_frac(0.1)+ ggplot(aes(x=SUM_BSB_CATCH,y=SUM_BSB_CATCH_v2)) + geom_point()
